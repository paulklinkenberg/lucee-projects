...
function _setCMSLinks()
{
	...
	if (sel && sel.options)
	{
		var arr = eval(myLinkList.responseText);
		if (arr.length)
		{
			sel.options[0] = new Option(" - choose website page", "");
			for (var i=0; i<arr.length; i++)
			{
				sel.options[i+1] = new Option(arr[i][0], arr[i][1]);
			}
			jQuery('#'+container+',#'+container+' select').show();
			_checkActivatedCMSLink();
		} else
			jQuery('#'+container).hide();
	} else
		setTimeout(_setCMSLinks, 500);
}
function _checkActivatedCMSLink()
{
	...
	{
		if (sel.options[i].value == currLink)
		{
						...
						[
							{
								...
								onChange : function(data){
									var myURL = data.data.value;
									var myLink = this.getDialog().getContentElement( 'info', 'url' );
									var myProtocol = this.getDialog().getContentElement( 'info', 'protocol' );
									myProtocol.setValue('<other>');
									myLink.setValue(myURL);
								}
							},
							{
								type : 'hbox',
